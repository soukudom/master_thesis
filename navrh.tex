...
s~identifikátorem odesílaným z~kolektoru. Důsledkem toho bude možnost detekční parametry
 ...
 \begin{itemize}
  ...
  specifikované uvnitř kulatých závorek. Zde bude možné určit minimální nebo maximální 
  \textit{soft} a~\textit{hard}
  limity, kterých daná položka aktuálního profilu může dosahovat. Pro \textit{soft} 
  limit bude dále možné 
  určit povolenou délku, po kterou může být nastavená hranice překročena (\textit{grace period}). 
...
v~záhlaví. Díky stejnému formátu zprávy bude možné veškerá data zasílat jedním společným rozhraním.
 ...
 Způsob komunikace mezi moduly je zachycen na Obrázku \ref{obr.muxDemuxDiag}. Vstupem do modulu
  \textit{Mux} jsou tři různé UniRec šablony, které jsou postupně zapouzdřeny do společného formátu
  a odeslány
  společným komunikačním rozhraním.
  Následně modul \textit{Demux} provede zpětné rozbalení zpráv na odpovídající komunikační rozhraní.
 
 \begin{figure}[ht]
   \begin{center}
   \includegraphics[scale=0.8]{pictures/mux&demux}
   \caption{Diagram komunikace modulů \textit{Mux} a \textit{Demux}}
