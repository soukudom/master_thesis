  ...
  \end{itemize}
   ...
   určující způsob výměny klíče:
    \begin{itemize}
     ...
     na připojované zařízení, které nemusí podporovat autentizaci. 
     \item \textbf{Out of Band}
     
     Veškeré klíče jsou vyměňovány odlišným komunikačním kanálem např. přes NFC (Near Field Communication).
     Celková bezpečnost této metody závisí na důvěryhodnoti použitého kanálu.
     \item \textbf{Passkey}
     
     Tato metoda vylepšuje \textbf{Just Works} o autentizaci, která spočívá v uživatelském zadání šesti 
     místného kódu. Pro ochran před odposlouchávání musí být použit algoritmus ECDH, který je dostupný
     až od verze 4.2.
