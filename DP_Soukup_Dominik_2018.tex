...
\abstractCS{
Hlavním cílem diplomové práce je analýza aktuálního stavu IoT sítí a~identifikace
bezpečnostních slabin
bezdrátových senzorových protokolů, které jsou detekovány vytvořeným nástrojem. V~analytické
...
používané protokoly včetně jejich bezpečnostních slabin. Dále je navržena architektura detekčního 
systému tak, aby bylo možné jeho budoucí rozšiřování a~nasazení i na IoT branách s~omezenými
prostředky. 

Výstupem této práce je modulární systém, který je možné konfigurovat a~přizpůsobit pro konkrétní
topologii. Výsledný nástroj je implementovaný v~jazyce C++ a~rozšiřuje již existující IoT bránu
BeeeOn \cite{beeeon} o~možnost detekce anomálií v~bezdrátových senzorových protokolech.
}
\abstractEN{
...
Kompletní zdrojové kódy systému NEMEA jsou také udržovany v~repozitářích služby Github. Pro
...
ukázce každá z~kompent používá standartní výstup, proto je nutné každý přípaz spustit v~samostatném
terminálovém okně.
\begin{verbatim}
...
\begin{figure}
	\dirtree{%
		...
		.3 mux\DTcomment{zdrojové kódy multiplexotu}.
		.3 demux\DTcomment{zdrojové kódy demultiplexoru}.
		.3 test\DTcomment{Sobory pro testování}.
